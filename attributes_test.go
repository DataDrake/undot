...
func TestParseAttributesSingle(t *testing.T) {
	...
	if n.Attributes["label"] == "" {
		t.Error("Attribute 'label' missing")
	}
	if n.Attributes["label"] != "1234" {
		t.Errorf("Attribute value does not match, found '%s'", n.Attributes["label"])
	}
	if dot != "" {
		t.Error("Dot string should be empty")
	}
...
func TestParseAttributesSingleQuotedMultivar(t *testing.T) {
	...
	if n.Attributes["label"] != "1234 | abcdef | hello" {
		t.Errorf("Attribute value does not match, found '%s'", n.Attributes["label"])
	}
	if dot != "" {
		t.Error("Dot string should be empty")
...
func TestParseAttributesMultiple(t *testing.T) {
	...
	if n.Attributes["type"] != "circle" {
		t.Errorf("Attribute value does not match, found '%s'", n.Attributes["circle"])
	}
	if n.Attributes["value"] == "" {
		t.Error("Attribute 'label' missing")
	}
	if n.Attributes["value"] != "1234 | bob" {
		t.Errorf("Attribute value does not match, found '%s'", n.Attributes["value"])
	}
	if dot != "" {
		t.Error("Dot string should be empty")
	}
